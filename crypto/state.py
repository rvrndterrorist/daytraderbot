...
class BotState:
    def __init__(self):
        ...
        self.coins = {}
        self.shared_pool_balance: float | None = None   # None = not using shared wallet
        self.fleet_starting_balance: float = 0.0
    ...
    def update_portfolio(self, symbol, balance_usd, position_size, position_entry_price,
                         total_value, trade_count, pnl_base=None):
        c = self.get_coin(symbol)
        with c._lock:
            ...
            # pnl_base: what to measure P&L against.
            # Default = starting_balance (non-shared wallet).
            # Shared wallet passes position cost so P&L is per-trade, not vs total pool.
            base = pnl_base if pnl_base is not None else c.starting_balance
            c.pnl_usd = round(total_value - base, 4) if base else 0.0
            c.pnl_pct = round((c.pnl_usd / base) * 100, 2) if base else 0.0
    ...
    def update_shared_pool(self, balance: float):
        with self._lock:
            self.shared_pool_balance = round(balance, 4)

    def snapshot(self) -> dict:
        with self._lock:
            snap = {"fleet": [c.snapshot() for sym, c in self.coins.items()]}
            if self.shared_pool_balance is not None:
                snap["shared_pool_balance"]    = self.shared_pool_balance
                snap["fleet_starting_balance"] = self.fleet_starting_balance
            return snap
