...
from crypto.shared_wallet import SharedWallet
...
def trading_loop(cfg: dict, exchange, symbol: str, notifier: Notifier, shared_wallet: SharedWallet | None = None):
        ...
        try:
            ...
            if sig == "buy":
                ...
                if res:
                    ...
                    if isinstance(trader, PaperTrader) and trader.is_out_of_funds():
                        bot_state.log(symbol, f"HOLD (Wallet empty — ${trader.balance_usd:.2f} remaining)")
                    else:
                        bot_state.log(symbol, f"HOLD (Already in position)")
            elif sig == "sell":
            ...
            if isinstance(trader, PaperTrader):
                if shared_wallet:
                    # Shared wallet: per-coin numbers show only that coin's position.
                    # balance_usd = what was spent to enter this position (0 if flat)
                    # total_value = what that position is worth right now (0 if flat)
                    # pnl_base   = position cost, so P&L = current value − what we paid
                    position_value = trader.position_size * price
                    cost = trader._position_cost_usd
                    bot_state.update_portfolio(symbol, cost,
                                               trader.position_size, trader.position_entry_price,
                                               position_value, trader.trade_count,
                                               pnl_base=cost if cost > 0 else None)
                    bot_state.update_shared_pool(shared_wallet.balance)
                else:
                    bot_state.update_portfolio(symbol, trader.balance_usd, trader.position_size,
                                               trader.position_entry_price,
                                               trader.portfolio_value(price), trader.trade_count)
            else:
...
def main():
    ...
    # One shared wallet for the whole fleet in paper mode.
    # All coins draw from and return to the same pool.
    if cfg.get("mode") != "live":
        wallet = SharedWallet(cfg.get("starting_balance", 20.0))
        bot_state.fleet_starting_balance = wallet.starting_balance
        bot_state.shared_pool_balance    = wallet.starting_balance
        print(f"  Shared paper wallet: ${wallet.starting_balance:.2f} across {len(fleet)} coins\n")
    else:
        wallet = None

    for sym in fleet:
        threading.Thread(target=trading_loop, args=(cfg, exchange, sym, notifier, wallet), daemon=True).start()
